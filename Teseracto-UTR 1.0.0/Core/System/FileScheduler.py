...
class FileScheduler:
    ...
    # Modificar métodos de carga de configuración
    def _cargar_config_email(self) -> Dict[str, Any]:
        try:
            config_path = path_manager.get_config_path("email_config.json") # MODIFICADO
    ...
    def _cargar_config_sms(self) -> Dict[str, Any]:
        try:
            config_path = path_manager.get_config_path("sms_config.json") # MODIFICADO
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    return json.load(f)
            return None
    ...
    def _enviar_archivos_pendientes(self):
        with self._lock:
            directorio = self.config.get("directorio_pendientes", str(path_manager.get_pendientes_usb_path())) # MODIFICADO
