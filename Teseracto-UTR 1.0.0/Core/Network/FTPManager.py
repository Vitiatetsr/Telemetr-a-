...
class FTPManager(IFileTransfer):
    ...
    def _conectar(self) -> bool:
        try:
            ...
            # CONEXIÓN SIMPLE SI secure_mode ES FALSE
            if not self.secure_mode:
                self.connection = ftplib.FTP(timeout=self.timeout)
                self.connection.connect(self.config["host"], self.port)
                self.connection.login(user=self.config["usuario"], passwd=self.config["clave"])
                return True
            
            # Solo intentar TLS si secure_mode es True
            try:    
                ...
                self.logger.warning(f"Fallo TLS, intentando sin cifrado: {e}")
                # Fallback a FTP simple
                self.connection = ftplib.FTP(timeout=self.timeout)
                self.connection.connect(self.config["host"], self.port)
                self.connection.login(user=self.config["usuario"], passwd=self.config["clave"])
                return True
