# Tesseract/GUI/Windows/DashboardWindow.py - VERSIÓN 100% FUNCIONAL
...
from Core.DataProcessing.Services import UnitConverter, ConfigProvider, FileNameGenerator, RecordFormatter, BitmaskConverter, USBRecordStorage
...
class DashboardWindow(QWidget):
    ...
    def init_storage_services(self):
        """Inicializa componentes para almacenamiento histórico"""
        try:
            self.config_provider = ConfigProvider(ConfigManager())
            self.file_name_gen = FileNameGenerator(self.config_provider)
            self.bitmask_converter = BitmaskConverter()
            self.record_formatter = RecordFormatter(
                self.config_provider,
                self.bitmask_converter
            )
            self.usb_storage = USBRecordStorage(
                self.config_provider,
                self.file_name_gen
            )
        except Exception as e:
            self.error_handler.log_error("DASH_STOR_INIT", f"Error inicializando almacenamiento: {e}")

    def setup_unit_conversion(self):
        """Configura conversión de unidades eficiente"""
        try:
            # CORRECCIÓN: Manejo seguro de registros
    ...
    def get_conversion_strategy(self, from_unit: str, to_unit: str):
        # Crear instancia para acceder a las estrategias
    ...
    def setup_data_acquisition(self):
        ...
        if not StateManager.is_system_ready():
            ...
            return  # ✅ No inicia hilos pero atributos ya existen
    ...
    def on_new_data(self, data):
        self.last_data = data
        self.last_update_time = time.time()
        self.store_historical_record(data)
        
    def store_historical_record(self, data):
        """Almacena registro histórico"""
        try:
            if not data or 'flujo_instantaneo' not in data:
                return
            
            record_type = "Medidor"
            formatted = self.record_formatter.format(
                record_type,
                data,
                self.medidor.perfil
            )
            self.usb_storage.store_record(record_type, formatted)
            
        except Exception as e:
            self.error_handler.log_error("DASH_STOR", f"Error almacenamiento: {e}")
    ...
    def refresh_unit_config(self):
        ...
        try:
            ...
            # Actualizar etiqueta de unidad inmediatamente
            self.lbl_instant.setText(f"0.00{self.display_unit}")
            
            # Si hay datos actuales, actulizar valor mostrado
    ...
    def update_ui(self):
        ...
        try:
            ...
            self.lbl_cod_error.setText(f"{cod_error:04X}")  # Formato hexadecimal 4 dígitos
    
            # Destacar si hay código de error
            if cod_error != 0:
